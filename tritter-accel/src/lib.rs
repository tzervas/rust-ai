...
pub use core::{
    ...
    quantization::{quantize_absmax, quantize_absmean, QuantizationResult, QuantizeConfig},
...
mod python_bindings {
    ...
    fn trainer_step<'py>(
        ...
        if step_info.needs_backward {
            ...
            if let Some(grad_dict) = gradients {
                ...
                for (name, arr) in grad_dict {
                    ...
                    let tensor = candle_core::Tensor::from_vec(data, shape.to_vec(), &device)
                        .map_err(|e| {
                            PyValueError::new_err(format!(
                                "Failed to create tensor for {name}: {e}"
                            ))
                        })?;
                ...
                inner.record_full_gradients(&tensor_grads).map_err(|e| {
                    PyValueError::new_err(format!("record_full_gradients failed: {e}"))
                })?;
            }
            result.insert("predicted_gradients".to_string(), py.None());
        } else {
            // PREDICT: get predicted gradients
            let predicted = inner.get_predicted_gradients().map_err(|e| {
                PyValueError::new_err(format!("get_predicted_gradients failed: {e}"))
            })?;
            ...
            result.insert(
                "predicted_gradients".to_string(),
                pred_dict.into_pyobject(py).unwrap().into_any().unbind(),
            );
        }
        ...
        result.insert(
            "speedup".to_string(),
            stats.speedup.into_pyobject(py).unwrap().into_any().unbind(),
        );
        result.insert(
    ...
    fn trainer_get_stats(
        ...
        result.insert(
            "cycles".to_string(),
            stats.cycles.into_pyobject(py).unwrap().into_any().unbind(),
        );
        result.insert(
            "speedup".to_string(),
            stats.speedup.into_pyobject(py).unwrap().into_any().unbind(),
    ...
    fn create_bitlinear<'py>(
        ...
        let weight_tensor =
            candle_core::Tensor::from_vec(weight_data, (out_features, in_features), &device)
                .map_err(|e| {
                    PyValueError::new_err(format!("Failed to create weight tensor: {e}"))
                })?;
        ...
        let bias_tensor = if let Some(b) = bias {
            ...
            Some(
                candle_core::Tensor::from_vec(bias_data, (out_features,), &device).map_err(
                    |e| PyValueError::new_err(format!("Failed to create bias tensor: {e}")),
                )?,
            )
        } else {
            None
    ...
    fn bitlinear_forward<'py>(
        ...
        let input_tensor =
            candle_core::Tensor::from_vec(input_data, (batch_size, in_features), &device).map_err(
                |e| PyValueError::new_err(format!("Failed to create input tensor: {e}")),
            )?;
...
mod tests {
    ...
    fn test_bitlinear_layer_creation() {
        ...
        let weight = candle_core::Tensor::randn(0.0f32, 1.0, (32, 64), &device).unwrap();
