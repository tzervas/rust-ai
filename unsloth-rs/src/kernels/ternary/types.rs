...
//!
//! ## Shared Types from trit-vsa
//!
//! This module re-exports core ternary types from `trit-vsa`:
//! - [`Trit`]: Single ternary value enum {N, Z, P}
//! - [`PackedTritVec`]: Efficient bitsliced vector storage
//!
//! ## Transformer-Specific Types
//!
//! This module provides transformer-specific types:
//! - [`TernaryPlanes`]: Low-level bitsliced storage for weights
//! - [`TernaryTensor`]: 2D weight tensor with per-row scales
//! - [`SparsityMetadata`]: Chunk-based sparsity tracking
//!
//! ## Representation
//!
...
// Re-export core ternary types from trit-vsa for use in unsloth-rs
...
impl TernaryPlanes {
    ...
    /// Convert to a `PackedTritVec` from trit-vsa.
    ///
    /// This enables interoperability with trit-vsa's VSA operations
    /// (bind, bundle, similarity).
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// use unsloth_rs::kernels::ternary::{TernaryPlanes, PackedTritVec};
    /// use trit_vsa::vsa;
    ///
    /// let planes = TernaryPlanes::new(64);
    /// let packed = planes.to_packed_trit_vec();
    /// let similarity = vsa::cosine_similarity(&packed, &other_packed);
    /// ```
    #[must_use]
    pub fn to_packed_trit_vec(&self) -> PackedTritVec {
        // PackedTritVec uses the same internal representation
    ...
    ///
    /// This enables using trit-vsa vectors in transformer operations.
    #[must_use]
...
mod tests {
    ...
    fn test_packed_trit_vec_dot_equivalence() {
        // Verify that dot product gives same result via both types
        ...
        // Dot via TernaryPlanes
        let dot_planes = planes_a.dot(&planes_b);

        // Convert to PackedTritVec and compute dot
        ...
        assert_eq!(dot_planes, 3); // 1*1 + (-1)*(-1) + 1*1 = 3
    }
}
